"""Исправляет ошибку https://github.com/snok/asgi-idempotency-header/issues/16."""
...
class IdempotencyHeaderMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if not (
            idempotency_key := Headers(scope=scope).get(
                self.idempotency_header_key.lower(),
            )
        ):
            return await self.app(scope, receive, send)
        ...
        if self.enforce_uuid4_formatting and not is_valid_uuid(idempotency_key):
            payload = {
                "detail": f"'{self.idempotency_header_key}' header value must be formatted as a v4 UUID",
            }
        ...
        async def send_wrapper(message: Message) -> None:
            if message["type"] == "http.response.start":
                ...
                is_content_type_set = "content-type" in response_state.response_headers
                is_application_json = (
                    response_state.response_headers["content-type"]
                    != "application/json"
                )
                if is_content_type_set and is_application_json:
